...
def remove_frames_containing_nan_or_nothing(data, label):
    ...
    return data, label
# kalman中需增加对nan帧的识别，作为一个时间步， 在run_instance_segment里面就需要
...
def radar_scenes_partitioned_data_generator(path_to_dataset: str, LSTM = False, non_static = False):
    ...
    if non_static:
        ...
        path = str(path_to_dataset) +'/shuffle/train_data_without_static.pickle'
        f = open(path, 'wb')
        pickle.dump(train_data, f)
        f.close()

        path = str(path_to_dataset) +'/shuffle/train_label_without_static.pickle'
        f = open(path, 'wb')
        pickle.dump(train_label, f)
        f.close()

        path = str(path_to_dataset) +'/shuffle/validation_data_without_static.pickle'
        f = open(path, 'wb')
        pickle.dump(validation_data, f)
        f.close()

        path = str(path_to_dataset) +'/shuffle/validation_label_without_static.pickle'
        f = open(path, 'wb')
        pickle.dump(validation_label, f)
        f.close()

        path = str(path_to_dataset) + '/shuffle/test_data_without_static.pickle'
        f = open(path, 'wb')
        pickle.dump(test_data, f)
        f.close()

        path = str(path_to_dataset) + '/shuffle/test_label_without_static.pickle'
        f = open(path, 'wb')
